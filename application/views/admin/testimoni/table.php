            ...
            <?php foreach($testimoni as $row): ?>
                ...
                <div class="panel panel-default a-panel <?php echo (md5($row['nama']) == $flashid ? 'panel-info' : ''); ?>" id="testimoni-<?php echo $row['id']; ?>">
                    <div class="panel-body">
                        <div class="a-panel-title"><strong><?php echo "#{$row['id']} - {$row['nama']}"; ?></strong></div>
                            ...
                            <div class="pull-left">
                                <div class="a-panel-tags" style="margin-top:3px">
                                    <?php foreach((array)explode(',', $row['tags']) as $tag): ?>
                                        <a href="<?php echo admin_url('testimoni/tag/'. trim($tag)); ?>"><span class="badge"><?php echo trim($tag); ?></span></a>
                                    <?php endforeach; ?>
                                </div>
                            </div>
